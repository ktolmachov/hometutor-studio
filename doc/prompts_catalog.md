...
- app/query_service.py (729 lines) → rg "^class\|^def " only, don't read full body
- app/tutor_prompts.py (87 lines, safe to read fully) — prompts are now distributed; no single monolithic prompts file
- app/knowledge_graph.py (1072 lines) → rg "^class\|^def " only
- doc/adr.md (925 lines) → read ONLY the status table or one ADR
- doc/architecture.md (745 lines) → read ONLY the module list, skip detail sections (now >600 lines)
...
|---|---|---|
| `app/tutor_prompts.py` (87 lines, safe to read fully) | read in full | designated prompt module; prompts now distributed across services |
| `doc/changelog.md` (2120 lines) | last 2-3 entries or append target only | history docs accumulate quickly |
| `tests/test_api.py` (1712, ~15k est tokens) | `rg "def test_<pattern>" tests/test_api.py` + one test case | full tests can exceed target budget |
| `doc/adr.md` (925, ~14k est tokens) | status table or one ADR only | full decision history is not phase input |
| `app/knowledge_graph.py` (1072, ~11k est tokens) | `rg "^class\|^def " app/knowledge_graph.py` | full-read is forbidden |
| `tests/test_query_service.py` (1145, ~11k est tokens) | 1-2 relevant tests or fixtures | avoid full test file |
| `app/query_service.py` (729, ~7k est tokens) | `rg "^class\|^def " app/query_service.py` + exact function | large orchestrator |
| `doc/architecture.md` (745, ~9k est tokens) | module list or one section only | now >600 lines — treat as large file |

> ⚠️ Цифры актуальны на 2026-05-17. Перед review проверить актуальный размер через `(Get-Content <file>).Count` в PowerShell (или `wc -l <file>` в Git Bash) — статические значения могут устареть между эпохами.
