...
> **Область этого документа:** markdown-промпты **team / agent workflow** (роли, планирование, аудит, оркестрация). Тексты LLM для runtime-приложения (RAG, тьютор, квизы и т.д.) — в `app/tutor_prompts.py` и inline в service-файлах; полный перечень сценариев и копипаста — в [Каталоге Промптов](prompts_catalog.md).
