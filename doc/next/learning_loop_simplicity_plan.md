...
Status: mixed: A1 done; A2 done; B1 done; B2/B3/C2/C3 proposed; C1 needs discovery. Это по-прежнему кандидаты; `backlog_registry.yaml` этим документом не меняется.
...
- **2026-07-11 (третья сверка, после реализации B1)** — B1 закрыт (`done`), commits
  `2d5401d`…`7148149` (`151`–`158`, основная логика в `151`/`154`). Верифицировано чтением
  кода и тестовым прогоном (`tests/test_knowledge_graph_counters.py` +
  `tests/test_mission_control_progressive.py` + `tests/test_navigation_visibility.py` +
  `tests/test_feature_registry.py` → **45 passed**). Реализация вышла ШИРЕ исходного
  scope плана: помимо `compute_kg_counters()` в `app/ui/knowledge_graph_d3.py` и канонического
  `is_lesson_node()` в `app/knowledge_graph.py` (data layer, не UI — лучше, чем предполагал
  план), фикс также накрыл третью, ранее не замеченную поверхность рассинхронизации —
  `get_progress_stats()` (использовалась `app/ui/pages/3_Мой_прогресс.py`,
  `app/ui/dashboards_progress.py`), которая тоже разбавляла «концепты» узлами-лекциями.
  Bundle-state бейдж (published/staging) из DoD тоже реализован
  (`_kg_bundle_state_badge()`). Graceful degradation сверх DoD: честное «Счётчики
  недоступны» вместо синтетических нулей при сбое одного из входных слоёв.
...
**Статус: `done`** — реализован в `hometutor`, commits `2d5401d`…`7148149` (`151`–`158`).
Раздел ниже оставлен как исторический контракт, не как pending-задача.

**Приоритет:** P1 · **Усилие:** малое–среднее, подтвердилось

**Проблема (была).** Mission Control и Knowledge Graph показывали разные значения под
похожими подписями: «концептов», «готово учить». Root cause — две независимые реализации
подсчёта над идентичным неотфильтрованным источником данных.

**Что реализовано (evidence, hometutor@7148149, верифицировано чтением кода + тестовым
прогоном, не принято на слово):**
- `app/ui/knowledge_graph_d3.py` (`compute_kg_counters`) — единый shared helper, возвращает
  `total`, `total_concepts` (без lesson-узлов), `total_lessons`, `frontier`
  (пересчитанный из mastery/prereqs/learned, не сырой флаг), `avg_mastery`, `learned`,
  `clusters` — согласованный набор для обоих экранов.
- `app/knowledge_graph.py` (`is_lesson_node`) — каноническое определение «это узел-лекция?»
  вынесено в data layer (не в UI, что даже лучше, чем предполагал план); `_is_lesson_node`
  в `knowledge_graph_d3.py` и `_is_lesson_concept` в `dashboards_graph.py` — это буквально
  тот же callable (`assert _is_lesson_node is is_lesson_node`), не независимые копии,
  которые могли бы снова разойтись.
- `app/ui/mission_control.py` (`render_kg_mission_card`) — переключён на
  `compute_kg_counters()` + `collect_kg_learned_set()`; регрессионный тест подтверждает,
  что старые независимые формулы (`total - lessons`, `d.get("frontier")`) полностью удалены
  из исходника функции.
- `app/ui/mission_control.py` (`_kg_bundle_state_badge`) — бейдж состояния бандла
  (published/staging), требовавшийся DoD, тоже реализован.
- **Шире исходного scope плана:** `get_progress_stats()` (используется
  `app/ui/pages/3_Мой_прогресс.py`, `app/ui/dashboards_progress.py`) — третья поверхность,
  не замеченная в исходном плане, тоже разбавляла «концепты» узлами-лекциями; теперь
  исключает их тем же каноническим `is_lesson_node()`.
- **Graceful degradation сверх DoD:** при сбое одного из входных слоёв (например,
  `typed_relations`) рендерится честное «Счётчики недоступны», а не синтетические нули —
  старый комбинированный try/except, который зануля́л все счётчики разом, удалён
  (регрессионный тест это фиксирует).
- `tests/test_knowledge_graph_counters.py` — исчерпывающий набор: границы lesson/concept
  (включая `lesson:`-префикс без явного `level`, case-insensitive), пересчитанный frontier
  vs устаревший сырой флаг, parity-тест `build_kg_payload["stats"] == compute_kg_counters(...)`,
  regression-guard на старые формулы. Прогнано вместе с
  `tests/test_mission_control_progressive.py` + `tests/test_navigation_visibility.py` +
  `tests/test_feature_registry.py`: **45 passed**.

**DoD (выполнено, проверено):**
- Одна версия графа даёт одинаковые counters на Mission Control и Knowledge Graph. ✅
- Разные версии графа явно промаркированы (`_kg_bundle_state_badge`). ✅
- Regression test не даёт вернуть независимый `total - lessons` расчёт. ✅

**Doc-sync:** не проверялся отдельно в рамках этой сверки — если добавлен
пользовательский bundle/source label на видимую поверхность, свериться с
`docs/user_guide.md` при следующем проходе.
...
3. ~~B1~~ — done in `2d5401d`…`7148149`.
4. **B2** и **B3** — следующие кандидаты: малые независимые cleanup packages, можно в любом
