...
**Ревизия v2 (2026-07-17):** сверено с @`643f59677` «263»; статусы волн проставлены в
заголовках, актуальный порядок — в разделе «Ревизия v2» ниже.
...
## Ревизия v2 — 2026-07-17 (hometutor @ `643f59677` «263»)
...
раздел конспекта/Obsidian нет; внутрь узла не провалиться; правила зала и смысл тура не
считываются. Reference-макет №18 выглядит сильнее реализации.
...
### Волна `wave-kg-memory-run-parity` (новый P0)
...
V2 (см. P1 ниже) остаётся обязательным: снять opt-in, матрица viewport'ов. Два
уточнения: (1) структурный дизайн-контракт скина U0 входит в обязательный смок (не
px-diff); (2) в DoD любой UI-волны — живой прогон running-артефакта, не только
Playwright-снимок fixture-страницы.

### Волна `wave-kg-doors` (новый P1) — ценность и двери

#### U1 «Карточка ценности узла» — показать всё, что payload уже знает

- **Problem.** `updateStopInfo` (`kg_3d_template.html:1407`) выводит 3–4 строки 11px:
...
### Волна `wave-kg-clarity` (новый P2) — правила и смысл тура

#### U4 «Правила зала + экскурсия»

- **Problem.** Правила зала — легенда 10px (`kg_3d_template.html:110-116`); смысл тура
...
**P0:** U0 → V2′ (обязательный gate + дизайн-контракт). **P1:** U1 → U2. **P2:** U3, U4.
**P3:** G4 (без изменений). Разделы G0–G3 ниже — исполненный контракт; сохранены как
справка по механике моста, действий, памяти и инвентаря.

| Что | Сейчас (@263) | После |
|---|---|---|
| Панель | пульт: ~10 кнопок, 10–12px | топбар + карточка остановки + 2 CTA (reference) |
| Читаемость | серые подписи 10–12px на чёрном | ≥12px + halo, семантическая палитра, AA |
| Ценность узла | 3 строки 11px | роль + W + mastery-кольцо + due/novel + дата квиза |
| Дверь в конспект | только `collect` в корзину | + открыть раздел в Obsidian из зала |
| Внутрь узла | нельзя (клик = фокус) | интерьер: разделы, связи, действия |
| Правила зала | легенда 10px | onboarding-оверлей + «?» |
| Тур | автопролёт без цели | экскурсия с причинами и финальным CTA |
| Визуальный gate | opt-in смок, 1 viewport | обязательный CI + матрица + дизайн-контракт |
...
### V2 ⬜ (не сделан: смок всё ещё opt-in; дополнен V2′ в «Ревизии v2») «Обязательный визуальный gate» — до промоута iframe
...
## Рекомендованный порядок (v1 — история; актуальный порядок см. «Ревизия v2»)

**P0:** G0 → G1. **P1:** V2 (gate, блокирует iframe) ‖ G2 → G3. **P2:** G4.
*(Исполнено @263: G0/G1/G2/G3 ✅, V2 ⬜. Метрики ниже — снимок «до v1», история.)*
