...
- reference **«Созвездие» / `world`** = **презентационный ярлык ровно одного режима —
...
- **Proposed.**
  1. Envelope: `{version:1, source:'kg3d', event_id, concept_id, action, ts}`,
     `action ∈ {'start','collect'}`. `event_id` — уникальный на нажатие (uuid/ULID,
     фикс. длина), даёт идемпотентность; `ts` — issued-at (epoch ms) для
     freshness-окна (защита от старого URL в новой сессии, где session-дедуп пуст).
  2. В 3D-шаблоне (только `host_mode==='embedded'`) **кнопки** карточки остановки шлют
     ...
     event_id, concept_id, action, ts})` — **полный envelope, включая `source` и `ts`**
     (событие без любого обязательного поля собственная валидация ниже отклонит). Обёртка компонента кодирует envelope в **`_kg3d`**
  ...
  3. Python-контракт `_kg3d` (security + idempotency, всё обязательно):
     - **сериализация:** `base64url(minified JSON envelope)` — один параметр, без
       вложенных разделителей; **предел длины параметра ≤ 600 символов**, длиннее —
       отбрасывается без обработки;
     - **валидация:** `version===1`, `source==='kg3d'`, `action ∈ {'start','collect'}`,
       **`concept_id ∈ node_ids`**, `event_id` — формат uuid/ULID фиксированной длины,
       **`ts` в freshness-окне** (напр. ≤ 10 минут; просроченное — молча отбросить и
       вычистить из URL);
     - **дедуп:** bounded set последних **64** обработанных `event_id` (FIFO, в
       `st.session_state`);
     - **порядок обработки строго `validate → consume → execute → ack`:** consume =
       удалить `_kg3d` из URL (`st.query_params`) **и** записать `event_id` в processed —
       **до** execute. Тогда исключение в `collect` не оставляет событие в URL и не
       повторяет запись при следующем rerun: пользователь видит ошибку-toast, ручной
       повтор кнопкой создаёт **новый** `event_id`. Старый URL в **новой** сессии
       session-дедупом не гасится (set пуст) — его отсекает freshness-окно по `ts`;
...
- **Proposed.**
  ...
  3. **Объект gate — production-артефакт, не reference-макеты:** рендерится
     `build_kg_3d_html(фиксированный fixture-payload)` (а после G0 — и embedded-страница
     компонента). Standalone-макеты — синтетические данные и другая state-machine: их
     snapshots — опциональная design-regression, устойчивость `kg_3d_template.html` они
     **не доказывают** и gate'ом C1 не являются. Цветовые различия не заменяют
     текст/форму состояния.
