...
- **Action bridge (child → Python):** зал шлёт намерение `{concept, action}` наружу —
...
- **Формат возврата — не ломать 2D.** Текущий `kg_d3_component` возвращает **строку**
  ...
  Нельзя молча заменить на `{concept, action}` — 2D-контракт сломается. Решение (выбрать
  ...
  concept-select (2D), объект `{concept, action}` = новый 3D, Python парсит оба.
...
- **Proposed.**
  1. Мост (по формату выше) несёт `{concept, action}` — механизм
  ...
  2. В 3D-шаблоне (только в `embedded`-режиме) **кнопки** карточки остановки шлют
     `postMessage({type:'hometutor:kg-action', concept, action})`. `action ∈
  ...
  3. Python кладёт результат в `st.session_state["kg_3d_action"] = {concept, action}`
     (UI-state, не per-user persistence) и показывает toast «принято: <action> ·
     <concept>».
...
- **DoD.** В продукте: клик по кнопке действия в карточке остановки embedded-зала
  ...
  per-user записи** (только UI-state). В `export`-режиме кнопки inert; offline-контракт
  (нет внешних `<script src>`) сохранён.
- **Doc-sync.** —
- **Effort.** День.
...
- **Proposed.**
  ...
  3. Контракт первого кадра: canvas не пустой, видны **все `route.length` остановок**
