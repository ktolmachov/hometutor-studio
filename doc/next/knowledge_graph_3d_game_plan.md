...
**Evidence-база и её свежесть.** Проверено на hometutor @ `085825fbb` «272» +
working **W0 quality** (2026-07-17). Ревизия v2 UI (U0–U4 + V2′) — **264–265**
поверх G0–G3 (257–263). Post-audit overlay **269–270**. **R1–R3 polish** —
**271–272** (topbar 64px + ⋯ menu, toast ack, hall lanes/underglow). **W0
(Q1–Q4/Q7)** — axis/nav collision, compass labels, `fitRouteCamera` mobile/desktop,
learner status line (см. `knowledge_graph_3d_world_vision.md`).
...
## Ревизия v2 — 2026-07-17 (hometutor @ «272»+W0; UI-скин @264–265)
...
### Статус волн v2 (UI Memory Run) — **DONE 2026-07-17** (+ post-audit @269–270)

| Волна | Статус | Evidence (hometutor) |
|---|---|---|
| **U0** Memory Run parity | ✅ | skin + **R1** topbar≤72 / primary ←▶→⋯; **R2** toast; **R3** hall lanes; **W0** axis above nav, fitRouteCamera mobile; stop list rank+✓ (@269) |
| **V2′** Gate | ✅ | design contract + viewport matrix always-on; **@270** live rank/✓; **@271–272** chrome/toast/hall asserts |
| **U1** Карточка ценности | ✅ | eyebrow/роль, W, mastery-ring, chips due/novel/✓/◆, дата квиза из `MASTERY_HISTORY` (приоритет history над `node.mastery===0`) |
| **U2** Дверь Obsidian | ✅ | `_concept_sections_view_model` → `__CONCEPT_SECTIONS__`; «Открыть раздел» embedded-only; export без `obsidian://` |
| **U3** Внутрь узла | ✅ | `openInterior` / «Внутрь» / 2-й клик; panel-state + fly-in ~200ms; Esc/✕ |
| **U4** Правила + экскурсия | ✅ | `#onboard` + «?»; легенда 10px убрана; Тур → «Экскурсия дня» |
| **G4** | ⬜ | не начат (P3) |

**Файлы write-set v2:** `app/ui/assets/kg_3d_template.html`, `app/ui/knowledge_graph_d3.py`,
`app/ui/dashboards_graph.py`, `app/ui/assets/kg_3d_component/index.html`,
`tests/test_knowledge_graph_counters.py`, `docs/user_guide.md` (+ мелкий race-fix
`kg_d3_component`). Post-audit write-set: `kg_3d_template.html` (@269),
`tests/test_knowledge_graph_counters.py` (@269–270).

**Живой прогон (агент, 2026-07-17 @265):** Streamlit `:8501` + API `:8000` → Knowledge Graph →
embedded 3D-зал: topbar/compass/routeOn, CTA height 40, start enabled, inventory,
sectionLinks≥1, interior_open, canvas nonBg>0.

**Пост-v2 аудит (агент, 2026-07-17 @269–270):** checklist U0/G0–G3/U1–U4/kill-switch —
structural + browser/live gates **соответствуют** Memory Run contracts. Единственный
P1: в списке остановок ✓ **заменял** номер → исправлено (номер всегда; ✓ overlay).
Pytest `tests/test_knowledge_graph_counters.py` + `test_knowledge_graph_d3_section.py`:
**78 passed** (design contract, viewport matrix, collect e2e, route_stop_done_check).
Literal pixel-perfect vs golden mockup HTML — **не проверен** (reference-файлы живут
в studio, не в runtime-репо; для gate используются structural/DOM contracts).

**Ops (вне KG-волн, 2026-07-17):** полный `ingest.py --reset` под
`EMBED_MODEL=text-embedding-qwen3-embedding-0.6b` — stored embed в
`ingestion_content_hashes.json` совпадает с config (15 files / 414 nodes); снят
mismatch `perplexity/pplx-embed-v1-0.6b` ↔ qwen3. API/UI на момент записи ops-шага
могут быть down — reindex CLI-independent.

**Исторический живой прогон владельца (до v2 UI, 2026-07-17 @263):** петля G0–G3
работала — восторга не было (аскетичный шаблон №17). Ниже диагноз **закрыт** волнами U0–U4.
...
### Актуальный порядок и метрики (v2)
...
|---|---|---|
| Панель | пульт: ~10 кнопок, 10–12px | ✅ топбар + карточка остановки + 2 CTA (Memory Run) |
| Читаемость | серые подписи 10–12px на чёрном | ✅ ≥12px + halo, `--kgx-*` палитра |
| Ценность узла | 3 строки 11px | ✅ роль + W + mastery-кольцо + due/novel + дата квиза |
| Дверь в конспект | только `collect` в корзину | ✅ + «Открыть раздел» `obsidian://` (embedded) |
| Внутрь узла | нельзя (клик = фокус) | ✅ интерьер: разделы, связи, действия |
| Правила зала | легенда 10px | ✅ onboarding-оверлей + «?» |
| Тур | автопролёт без цели | ✅ «Экскурсия дня» + причины в карточке + финальный CTA |
| Визуальный gate | opt-in смок, 1 viewport | ✅ structural always-on + viewport matrix + **live rank/✓ overlay DOM** (@270) |
| Список done-остановок | (до @269) ✓ **заменял** номер | ✅ номер всегда + `.stop-check` overlay; canvas badge rank+✓ |
...
*(Исполнено: G0–G3 @257–263 ✅; V2/V2′ + U0–U4 @264–265 ✅; U0/G2 overlay audit
@269–270 ✅; G4 ⬜. Метрики ниже — снимок «до v1», история + актуализация @270.)*
...
| Что | Сейчас | После |
|---|---|---|
| Действие из зала | невозможно (моста нет) | ✅ клик по **кнопке** → `start`/`collect` (G0/G1) |
| Добыча со сцены | невозможно (у 2D дверь есть, у зала нет) | ✅ «В конспект» + «Открыть раздел» Obsidian (G1/U2, embedded) |
| Финал тура | «Маршрут дня завершён» (тупик) | ✅ следующий непройденный + действие; «Экскурсия дня» (U4) |
| 3D в продукте | download-only | ✅ embedded iframe C1 + Memory Run skin (U0); export download |
| Память в зале | нет | ✅ ✓ по квизам, снимок с датой (G2); **ранг не затирается ✓** (@269–270) |
| Визуальный смок | opt-in skip, 1 viewport | ✅ structural + viewport matrix + live overlay DOM (V2′ @270) |
| UI / ценность / внутри | пульт №17 | ✅ U0–U4 (карточка, двери, интерьер, onboarding) |
| Подписей в первом кадре | ≤8 | ✅ ≤8 (скин не добавляет сущностей) |
| G4 прогрессия/реплей | — | ⬜ не начат |
| Индекс / embed runtime | (ops) mismatch pplx ↔ qwen3 | ✅ reindex `ingest.py --reset` @2026-07-17 → qwen3 stored=config |
...
ИИ-Хранитель, антагонисты из данных, рассвет по прогрессу) + аудит качества скина
@270 (Q1–Q9, включая mobile-fit и компас): см.
`knowledge_graph_3d_world_vision.md` (2026-07-17). Kill switch там ревизован (v3):
LLM разрешён через provider layer с кэшем/бюджетом/деградацией; домен и первый
route-кадр остаются неприкосновенными.
