...
**Evidence-база и её свежесть.** Проверено на hometutor @ `3d6fe0a94` «270»
(2026-07-17). Ревизия v2 UI (U0–U4 + V2′) — коммиты **264–265** поверх G0–G3
(257–263). **Пост-v2 аудит + fix U0/G2 overlay** — **269–270** (номер остановки
остаётся видимым; ✓ — `.stop-check` overlay; live DOM gate в visual smoke).
...
## Ревизия v2 — 2026-07-17 (hometutor @ `3d6fe0a94` «270»; UI-скин @264–265)
...
|---|---|---|
| **U0** Memory Run parity | ✅ | `kg_3d_template.html`: `--kgx-*`, `#topbar` 64px, panel 314px, CTA `kgx-action-primary` ≥40px, компас, иконки на сцене; пульт №17 снят; **список маршрута: номер остановки + `.stop-check` ✓ overlay** (@269) |
| **V2′** Gate | ✅ | `test_3d_memory_run_design_contract_static` always-on; `test_3d_visual_smoke_viewport_matrix` always-on (Playwright; opt-out `HT_SKIP_KG_3D_VISUAL=1`); **@270:** live DOM — rank text, `.stop-check` absolute, topbar≥64 / side~314 desktop, no external scripts |
...
**Post-audit (@269–270):** U0/G2 rank+✓ overlay regression ✅ · V2′ live DOM assertions ✅.  
**Осталось:** P3 G4 ⬜. Разделы G0–G3 ниже — исполненный контракт (справка по мосту,
действиям, памяти, инвентарю).
