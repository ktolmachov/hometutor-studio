...
- **Component args / view-model (Python → зал):** продукт передаёт в шаблон входные
  данные (payload, флаг режима, множество «в корзине», дату снимка) — это **не** мост,
  а вход компонента. Оба режима получают args; в export они запечены, в embedded — живые.

**Про «мост в export».** Мостовой JS может физически присутствовать в шаблоне, но в
`export`-режиме он **inert**: кнопки действий не подключены, `postMessage` в отсутствие
родителя-слушателя — no-op. Контракт export — «действия не срабатывают» (inert/read-only),
а не «строк моста нет в файле». Offline-контракт №17 (нет внешних `<script src>`) при этом
сохраняется: внутренний `postMessage` его не нарушает.
...
- **Proposed.**
  1. Мост несёт версионированный envelope
     `{version:1, source:'kg3d', event_id, concept_id, action}` — механизм
     `setComponentValue` + `_kgc`-rerun уже проверен, изобретать query-params не нужно.
     `event_id` генерируется один раз на нажатие и позволяет Python дедуплицировать
     одно действие при повторном Streamlit rerun.
