...
**Evidence-база и её свежесть.** Проверено на hometutor @ `7a0e67eaf` «265»
(2026-07-17). Ревизия v2 UI (U0–U4 + V2′) реализована коммитами **264–265** поверх
G0–G3 (257–263). **Line-refs в исторических разделах G0–G3 — ориентиры на момент
реализации, не контракт**; сверять по имени функции. Актуальный порядок и статусы —
в разделе «Ревизия v2» ниже.
...
## Ревизия v2 — 2026-07-17 (hometutor @ `7a0e67eaf` «265»)
...
### Статус волн v2 (UI Memory Run) — **DONE 2026-07-17**

| Волна | Статус | Evidence (hometutor) |
|---|---|---|
| **U0** Memory Run parity | ✅ | `kg_3d_template.html`: `--kgx-*`, `#topbar` 64px, panel 314px, CTA `kgx-action-primary` ≥40px, компас, иконки на сцене; пульт №17 снят |
| **V2′** Gate | ✅ | `test_3d_memory_run_design_contract_static` always-on; `test_3d_visual_smoke_viewport_matrix` always-on (Playwright; opt-out `HT_SKIP_KG_3D_VISUAL=1`, не opt-in) |
...
**Живой прогон (агент, 2026-07-17):** Streamlit `:8501` + API `:8000` → Knowledge Graph →
embedded 3D-зал: topbar/compass/routeOn, CTA height 40, start enabled, inventory,
sectionLinks≥1, interior_open, canvas nonBg>0. Pytest `test_knowledge_graph_counters.py`:
65 passed (включая viewport matrix).
...
| Визуальный gate | opt-in смок, 1 viewport | ✅ structural always-on + viewport matrix (opt-out `HT_SKIP_…`) |
...
- **Tests.** `test_3d_memory_run_design_contract_static`,
  `test_3d_visual_smoke_viewport_matrix` в `tests/test_knowledge_graph_counters.py`.
...
*(Исполнено: G0–G3 @257–263 ✅; V2/V2′ + U0–U4 @264–265 ✅; G4 ⬜. Метрики ниже —
снимок «до v1», история.)*
...
| Память в зале | нет | ✅ ✓ по квизам, снимок с датой (G2) |
| Визуальный смок | opt-in skip, 1 viewport | ✅ structural + viewport matrix always-on (V2′) |
