...
«≥2 подтверждённых искажения смысла источника, прошедших все структурные
...
1. **Семпл 15 (5 тем × 3 повтора)** по рубрике №26 v1.0 с независимой
   ...
   («уголовное наказание» класс обязан не пройти в mastery или прийти с ярлыком);
...
- Finding 1 (`r1/guardrails#3`, «уголовное наказание для системы») не должен
  пройти в mastery без ярлыка (детерминированный gate это закрывает —
  exact-match цитаты обманывается сложнее, чем overlap);
