...
только **W0′ + W1** (+ опционально **W2a**). Остальное — каталог кандидатов до
owner-решений (§5 home, §6 LLM budget, §6F design spike, §7 Разлом data audit).
...
| R1 chrome · R2 toast · R3 hall lanes | ✅ | @271–272 |
...
- **R1 (ex-Q4 residual). Desktop: маршрут всё ещё прижат к верхней половине**;
...
- **R2 (ex-Q2 residual). Две легенды навигации:** floor «ВХОД / СЕГОДНЯ / ДАЛЬШЕ»
...
- **R3 (ex-Q3 residual). Mobile 390×844:** маршрут в кадре в целом держится, но
...
- **R4 (ex-Q7). Dev-hint footer** (`12 узлов · маршрут 6/6 · режим route · export`)
  — убрать из learner surface; перенести в `?`-диалог / `title` / diagnostics.
- **R5. Chip copy bug:** `due ${n.due}` при boolean даёт «due true»; `novel` —
  developer English. Fix: learner chips (`пора повторить` / `новое`), без raw
  boolean/english keys.
- **R6. Progress ring @0%** — трек есть, но визуально почти «пустой круг»;
  усилить track contrast (WCAG-ish на тёмном фоне).
- **R7. Export CTA hierarchy** — disabled primary/secondary выглядят одинаково
...
**Прорыв уровня качества = W0′ (R1–R7) + W1 «Рассвет» (§8).**  
...
| Рассвет / небо | `quiz_seen / route.length` | ночь (0/N) |
...
| «Развеять туман» / review | **`review`** (новый whitelist) | Flashcards | flashcards focus/queue keys (discovery в W2b) |
...
- кэш обязателен; ключ минимум
  `(scenario, snapshot_date|day, locale, route_fingerprint|concept_set_hash)`;
...
### 6.2 LLM budget appendix (kill switch v3 — числовой)

| Параметр | Значение v1 | Примечание |
|---|---|---|
| Max LLM calls / UI-session | **≤ 4** | сверх → только static degrade |
| Max completion tokens / call | **≤ 400** | A: ≤6×2 предл.; B: короткий список |
...
Ослабление «LLM = стоп» (v1/v2) → «LLM = opt-in narrative layer» (v3) **только**
...
| Волна | Содержание |
|---|---|
| W3a | infra only: cache harness, budget counters, degrade smoke, prompts package stub |
| W3b | scenario A |
| W3c | scenario B |
...
- **Рассвет (hero):** небо = `quiz_seen/route.length`. 0/N ночь → N/N золото.
...
|---|---|---|
| Маршрут | **Тропа рассвета** — чистая тропа, небо по quiz-прогрессу | gradient от `quiz_seen/route.length` |
...
|---|---|---|---|---|---|---|
| **W0′** Residual polish | R1–R7 §1.2 | fitCamera vertical fill; одна легенда compass/axis; mobile overlap; hide `#hint`; learner chips; ring track; export CTA copy | P0 | ≤1 дня | `kg_3d_template.html`, counters tests | visual smoke viewports; chip text asserts; no critical overlap |
| **W1** Рассвет и фонари | нет «живого» quiz-неба | sky gradient + lanterns from quiz-seen (§8) | P0 | ≤1 дня | `kg_3d_template.html` (+ tests) | 0/N night ≠ N/N dawn visually; reduced-motion ok; route frame still clean |
...
| **W6c** Летописец text on G4.2 | Keeper G prose | P3 | W3a; G4.2 exists |
| **W6d** Стройка / Architect E | freshness + optional LLM advice | P3 | freshness signal audit |
| **G4.3** фото дня | privacy DoD game plan | P3 | отдельное owner privacy review |
...
| Route completeness feel | visual: 6/6 → dawn after W1 | qualitative + screenshot gate |
| Return-to-hall | optional local counter session_state «hall_returns» (UI-only) | рост после W4b — не vanity cloud analytics |
| LLM budget integrity | counters + tests | 0 domain writes; ≤4 calls/session |
