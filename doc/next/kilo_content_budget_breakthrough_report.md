...
**Дата:** 2026-07-22  
**Evidence:** `logs/kilo_relay.jsonl` (last 40 + 3 новых записи с `content_stats`), `logs/kilo_content_report.json`
...
| Не трогать | Почему |
|------------|--------|
| Trim `messages[]` в relay | ломает tool/reasoning continuity |
...
- **msgs growth:** даже с дисциплиной read-set длинный чат снова уходит к 100k+ `in`; нужен новый чат + relay guard soft/hard.
...
.\.venv\Scripts\python.exe -m pytest tests/test_kilo_prompt_stats.py tests/test_kilo_proxy_relay.py -q
```
