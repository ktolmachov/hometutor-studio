...
```yaml
last_review:
  code_sha: <CODE_ROOT HEAD на котором закрыт предыдущий review>
  docs_sha: <DOCS_ROOT HEAD на котором закрыт предыдущий review>
  date: <YYYY-MM-DD>
...
findings:
  - id: AR-2026-04-12-001            # stable id, не меняется между review
    ...
    title: "Hardcoded prompt outside app/tutor_prompts.py"
...
- Scope фазы = diff против `last_review.code_sha`/`docs_sha` (per root) + changed modules из `git -C <root> diff --name-only <sha>..HEAD` + модули из persists-findings этой фазы. Остальное в фазе **не сканируется**.
...
2. Compute incremental scope from BOTH roots:
   ...
   - If a stored sha is missing or dangling in its repo, treat that root as
     first-run for this phase (full scope) and flag the stale sha in the report.
...
> ⚠️ Перед review проверить актуальный размер каждого файла через `(Get-Content <file>).Count` (PowerShell) или `wc -l <file>` (Git Bash). app/* files are under CODE_ROOT. Никаких зашитых чисел строк — они устаревают между эпохами.
...
- Finding history precedence:
  ...
  2) `last_review.report_files` (list) from baseline — reference reports for previous cycle.
...
1. `doc/archive/arch_review_baseline.yaml` отсутствует → создать с пустым `findings: []` и `last_review.code_sha: <CODE_ROOT HEAD>`, `docs_sha: <DOCS_ROOT HEAD>`.
