...
5. **История по умолчанию не обрезается** (`KEEP_LAST_MESSAGES=0`). Для длинных agent-loop — opt-in Tier B (`KILO_RELAY_CLOUD_BUDGET_KEEP_LAST_MESSAGES`, `MAX_TOOL_RESULT_CHARS`; § Tier B ниже). Иначе при `msgs`≫15 / body≫`hard_block_body_chars` (**110000**) — новый чат. Неизвестный `SLIM_MODE` → fail-fast при старте.
...
**Важно:** в `cloud_budget` переменные `KILO_RELAY_REPLACE_CURSOR_SYSTEM`, `KILO_RELAY_KEEP_LAST_MESSAGES` и `KILO_RELAY_MAX_TOOL_RESULT_CHARS` **не работают** — только одноимённые `KILO_RELAY_CLOUD_BUDGET_*` (override, не fallback; см. `_kilo_relay_compress.py:593-594` generic-чтение vs `:626-627` cloud_budget override).
