...
5. **История (`msgs`) compress не обрезает.** При `msgs`≫15 / body≫`hard_block_body_chars` (SSoT кода: **110000**) — новый чат / короче agent-loop. Неизвестный `SLIM_MODE` → fail-fast при старте (не молчаливый `local`).
