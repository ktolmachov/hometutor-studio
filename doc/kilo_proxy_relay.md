...
**Актуализация:** 2026-07-22 (DeepSeek-preset + фикс regressions, UTF-8 stdio, выводы [architecture review](next/kilo_relay_daily_architecture_review_2026-07-21.md) §18-27 — начинать с блока «Текущий статус (актуально)» в начале того файла).
...
⚠ **Не закрыто, вне контроля relay:** DeepSeek требует, чтобы `reasoning_content` из ответа с tool call был передан обратно в истории сообщений на **каждом** следующем шаге разговора — иначе API вернёт `HTTP 400`. Relay проксирует историю как есть, ничего не проверяет и не чинит. Kilo не писался под конвенцию `reasoning_content` DeepSeek, поэтому многошаговый agentic tool-loop через DeepSeek preset может сломаться на первом же tool round-trip. **Не проверено end-to-end.**
