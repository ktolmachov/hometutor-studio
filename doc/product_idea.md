...
Проект `hometutor` — локальный RAG-сервис для работы с личной базой знаний, заметками и проектной документацией. Он индексирует документы из папки `data/`, находит релевантные фрагменты и формирует ответ с привязкой к источникам.
