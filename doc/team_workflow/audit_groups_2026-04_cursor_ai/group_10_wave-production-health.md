...
- Packages: 2
- Waves: wave-production-health
- CJM moments: #10 Retrieval trust, #2 First Answer
- User stories in April index: US-12.4
- CI miss: 0
- US miss: 1
...
|---:|---|---|---|---|---|---|---|
| 1 | epoch-latency-slo-gate | 2026-04-25 | wave-production-health | OK | MISS | #2 First Answer | p95 latency CI gate интегрирован в pre-merge workflow; порог из tests/eval/thresholds.json. |
| 2 | epoch-llm-regression-baseline | 2026-04-25 | wave-production-health | OK | OK | #2 First Answer, #10 Retrieval trust | Full LLM regression suite с golden baselines; nightly CI job; нет silent regression на merge. |

## Strong Relations Inside This Group

- `epoch-latency-slo-gate` <-> `epoch-llm-regression-baseline`: depends_on:epoch-latency-slo-gate, wave:wave-production-health

## Coverage Completion Prompt

Use this file as a standalone Cursor AI prompt for completing DoD test coverage for `group_10_wave-production-health.md`.
It follows `archive/doc_team_workflow/audit_coverage_prompt_2026-04_cursor_ai.md`, but is scoped to this group only.

## Inputs

- This group file: `doc/team_workflow/audit_groups_2026-04_cursor_ai/group_10_wave-production-health.md`
...
|---|---|---|---|
| `epoch-latency-slo-gate` | dod, cli/schema | GAP_E2E_OR_UI_SMOKE | first-answer learner assertion |
| `epoch-llm-regression-baseline` | dod, cli/schema | GAP_E2E_OR_UI_SMOKE | source-trust/citation assertion |
...
1. `epoch-latency-slo-gate`
2. `epoch-llm-regression-baseline`
...
```powershell
Select-String -Path doc/closed_iterations.md -Pattern "epoch\-latency\-slo\-gate|epoch\-llm\-regression\-baseline"
Select-String -Path doc/cjm.md -Pattern "US\-12\.4|\#10\ Retrieval\ trust|\#2\ First\ Answer"
Select-String -Path doc/team_workflow/audit_groups_2026-04_cursor_ai/coverage_dod_analysis.md -Pattern "epoch\-latency\-slo\-gate|epoch\-llm\-regression\-baseline"
```

Before editing tests for a package, run a scoped read-set check:

```powershell
.\.venv\Scripts\python.exe scripts/check_readset.py doc/backlog_registry.yaml doc/user_stories_index.json doc/team_workflow/audit_groups_2026-04_cursor_ai/group_10_wave-production-health.md
```
...
`archive/team_artifacts/audit_2026-04/group_10_dod_coverage_report.md`

Required format:

```markdown
# DoD Coverage Completion ? Group 10
