...
|---:|---|---|---|---|---|---|---|
| 1 | epoch-sync-restore-wizard | 2026-04-22 | wave-sync-export | OK | MISS | infra | Restore wizard в Settings: загрузка файла, валидация sync_version, preview row counts, confirm |
| 2 | epoch-sync-multidevice | 2026-04-22 | wave-sync-export | OK | MISS | infra | Multi-device parity: экспорт/импорт bundle + merge-конфликт policy |

## Strong Relations Inside This Group

- `epoch-sync-restore-wizard` <-> `epoch-sync-multidevice`: depends_on:epoch-sync-restore-wizard, wave:wave-sync-export

## Coverage Completion Prompt

Use this file as a standalone Cursor AI prompt for completing DoD test coverage for `group_06_wave-sync-export.md`.
It follows `archive/doc_team_workflow/audit_coverage_prompt_2026-04_cursor_ai.md`, but is scoped to this group only.

## Inputs

- This group file: `doc/team_workflow/audit_groups_2026-04_cursor_ai/group_06_wave-sync-export.md`
...
|---|---|---|---|
| `epoch-sync-restore-wizard` | unit | GAP_DOD_COMMAND | CLI/schema/policy regression for infra behavior |
| `epoch-sync-multidevice` | unit | GAP_DOD_COMMAND | CLI/schema/policy regression for infra behavior |
...
1. `epoch-sync-restore-wizard`
2. `epoch-sync-multidevice`
...
```powershell
Select-String -Path doc/closed_iterations.md -Pattern "epoch\-sync\-restore\-wizard|epoch\-sync\-multidevice"
Select-String -Path doc/cjm.md -Pattern "infra"
Select-String -Path doc/team_workflow/audit_groups_2026-04_cursor_ai/coverage_dod_analysis.md -Pattern "epoch\-sync\-restore\-wizard|epoch\-sync\-multidevice"
```

Before editing tests for a package, run a scoped read-set check:

```powershell
.\.venv\Scripts\python.exe scripts/check_readset.py doc/backlog_registry.yaml doc/user_stories_index.json doc/team_workflow/audit_groups_2026-04_cursor_ai/group_06_wave-sync-export.md
```
...
`archive/team_artifacts/audit_2026-04/group_06_dod_coverage_report.md`

Required format:

```markdown
# DoD Coverage Completion ? Group 06
