...
- Packages: 2
- Waves: wave-agentic-tutor-depth
- CJM moments: #3 Transition to tutor, #8 Learning plan, #9 Master
- User stories in April index: US-14.4, US-4.1, US-4.2, US-8.1, US-8.2, US-9.2
- CI miss: 0
- US miss: 0
- CJM cross-links to other groups: #3 Transition to tutor -> groups [3, 7, 9, 15]; #8 Learning plan -> groups [3, 4, 9]; #9 Master -> groups [1, 3, 9]

## Packages

| # | Package ID | Date | Wave | CI | US | CJM | Title |
|---:|---|---|---|---|---|---|---|
| 1 | epoch-mastery-gap-routing | 2026-04-25 | wave-agentic-tutor-depth | OK | OK | #3 Transition to tutor, #8 Learning plan | Orchestrator маршрутизирует следующую тему исходя из реального mastery/gap, не random_next. |
| 2 | epoch-concept-remediation-step | 2026-04-25 | wave-agentic-tutor-depth | OK | OK | #3 Transition to tutor, #9 Master | Tutor предлагает конкретные шаги исправления по каждому concept gap в плане. |

## Strong Relations Inside This Group

- `epoch-mastery-gap-routing` <-> `epoch-concept-remediation-step`: depends_on:epoch-mastery-gap-routing, wave:wave-agentic-tutor-depth

## Coverage Completion Prompt

Use this file as a standalone Cursor AI prompt for completing DoD test coverage for `group_09_wave-agentic-tutor-depth.md`.
It follows `archive/doc_team_workflow/audit_coverage_prompt_2026-04_cursor_ai.md`, but is scoped to this group only.

## Inputs

- This group file: `doc/team_workflow/audit_groups_2026-04_cursor_ai/group_09_wave-agentic-tutor-depth.md`
...
|---|---|---|---|
| `epoch-mastery-gap-routing` | dod, unit | GAP_E2E_OR_UI_SMOKE | Tutor transition/learning-loop assertion |
| `epoch-concept-remediation-step` | dod, unit | GAP_E2E_OR_UI_SMOKE | Tutor transition/learning-loop assertion |
...
- Group intent focus: #3 Transition to tutor, #8 Learning plan, #9 Master.
...
1. `epoch-mastery-gap-routing`
2. `epoch-concept-remediation-step`
...
```powershell
Select-String -Path doc/closed_iterations.md -Pattern "epoch\-mastery\-gap\-routing|epoch\-concept\-remediation\-step"
Select-String -Path doc/cjm.md -Pattern "US\-14\.4|US\-4\.1|US\-4\.2|US\-8\.1|US\-8\.2|US\-9\.2|\#3\ Transition\ to\ tutor|\#8\ Learning\ plan|\#9\ Master"
Select-String -Path doc/team_workflow/audit_groups_2026-04_cursor_ai/coverage_dod_analysis.md -Pattern "epoch\-mastery\-gap\-routing|epoch\-concept\-remediation\-step"
```

Before editing tests for a package, run a scoped read-set check:

```powershell
.\.venv\Scripts\python.exe scripts/check_readset.py doc/backlog_registry.yaml doc/user_stories_index.json doc/team_workflow/audit_groups_2026-04_cursor_ai/group_09_wave-agentic-tutor-depth.md
```
...
`archive/team_artifacts/audit_2026-04/group_09_dod_coverage_report.md`

Required format:

```markdown
# DoD Coverage Completion ? Group 09
