...
- Packages: 2
- Waves: wave-answer-quality-eval
- CJM moments: #10 Retrieval trust, #2 First Answer
- User stories in April index: US-3.1, US-3.2
- CI miss: 0
- US miss: 1
- CJM cross-links to other groups: #10 Retrieval trust -> groups [8, 10, 13, 15]; #2 First Answer -> groups [3, 8, 10, 11, 15]

## Packages

| # | Package ID | Date | Wave | CI | US | CJM | Title |
|---:|---|---|---|---|---|---|---|
| 1 | epoch-aqe-corpus-choice | 2026-04-25 | wave-answer-quality-eval | OK | MISS | #2 First Answer, #10 Retrieval trust | Выбор и формирование golden set для AQE: synthetic corpus или real-data; AQE-R remediation decision. |
| 2 | epoch-answer-quality-baseline | 2026-04-25 | wave-answer-quality-eval | OK | OK | #2 First Answer | Eval pipeline + baseline score + CI gate; scripts/run_aqe.py; результат ≥ 80% green в pre-merge. |

## Strong Relations Inside This Group

- `epoch-aqe-corpus-choice` <-> `epoch-answer-quality-baseline`: depends_on:epoch-aqe-corpus-choice, wave:wave-answer-quality-eval

## Coverage Completion Prompt

Use this file as a standalone Cursor AI prompt for completing DoD test coverage for `group_08_wave-answer-quality-eval.md`.
It follows `archive/doc_team_workflow/audit_coverage_prompt_2026-04_cursor_ai.md`, but is scoped to this group only.

## Inputs

- This group file: `doc/team_workflow/audit_groups_2026-04_cursor_ai/group_08_wave-answer-quality-eval.md`
...
|---|---|---|---|
| `epoch-aqe-corpus-choice` | dod, cli/schema | GAP_E2E_OR_UI_SMOKE | source-trust/citation assertion |
| `epoch-answer-quality-baseline` | dod, cli/schema | GAP_E2E_OR_UI_SMOKE | first-answer learner assertion |
...
- Group intent focus: #10 Retrieval trust, #2 First Answer.
...
1. `epoch-aqe-corpus-choice`
2. `epoch-answer-quality-baseline`
...
```powershell
Select-String -Path doc/closed_iterations.md -Pattern "epoch\-aqe\-corpus\-choice|epoch\-answer\-quality\-baseline"
Select-String -Path doc/cjm.md -Pattern "US\-3\.1|US\-3\.2|\#10\ Retrieval\ trust|\#2\ First\ Answer"
Select-String -Path doc/team_workflow/audit_groups_2026-04_cursor_ai/coverage_dod_analysis.md -Pattern "epoch\-aqe\-corpus\-choice|epoch\-answer\-quality\-baseline"
```

Before editing tests for a package, run a scoped read-set check:

```powershell
.\.venv\Scripts\python.exe scripts/check_readset.py doc/backlog_registry.yaml doc/user_stories_index.json doc/team_workflow/audit_groups_2026-04_cursor_ai/group_08_wave-answer-quality-eval.md
```
...
`archive/team_artifacts/audit_2026-04/group_08_dod_coverage_report.md`

Required format:

```markdown
# DoD Coverage Completion ? Group 08
