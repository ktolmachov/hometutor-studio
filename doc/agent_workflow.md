...
Этот документ описывает, как использовать coding agents в `hometutor` так, чтобы они помогали быстрее, а не создавали конфликты и скрытые регрессии.
...
Для `hometutor` безопасный дефолт такой:
