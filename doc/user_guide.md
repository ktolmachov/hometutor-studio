# hometutor — учебный центр из твоей папки
...
Этот гайд — главная карта продукта. Если [quickstart.md](quickstart.md) ведёт тебя по 10 шагам, то здесь ответ на вопрос: **что умеет hometutor, куда нажимать и когда какая часть особенно полезна**.
...
| **hometutor** | объединяет поиск, тьютора, квизы, карточки, прогресс | требует локальной установки |
...
Если в `data/ML-Course/` лежит курс из 20 лекций, hometutor может превратить папку в учебное пространство:
...
# явный alias
.\scripts\Smoke-Hometutor-LlamaCpp.ps1 -Model qwen3.6-27b
```
...
Скрипт делает один `POST /ask` (`profile=quality`) и выводит `WARMUP_RAG=PASS` с таймингами. Это не заменяет `Smoke-Hometutor-LlamaCpp.ps1`, но снимает cold tax с первого пользовательского запроса.

**llama.cpp / MTP (опционально):** предупреждение `no implementations specified for speculative decoding` в логах сервера не блокирует работу (~190 t/s на qwopus35b без spec-decoding). Настройка draft-модели — на стороне вашего `llama-server` launch script, не hometutor.
...
| **VPS (RUVDS / Hetzner)** | UI + FastAPI за Nginx, HTTPS, опционально CI deploy | [deploy/nginx/hometutor.conf.example](../deploy/nginx/hometutor.conf.example), [presentations/defense_deploy_plan.md](presentations/defense_deploy_plan.md) |
...
hometutor нужен не для того, чтобы «поговорить с PDF». Он нужен, чтобы пройти полный учебный цикл:
