...
Срез на 2026-07-19 после добавления №26. Теги пересекаются: один разбор может входить в несколько
...
**Здоровье серии.** Канонических разборов — 26; ещё 3 HTML-файла являются
...
|---|---|---|
| Wiring-status North star | `mixed` | №1–23 не аудированы; №24 — `wire-in-P0`; №25 SGAR — `not measurable` до E2; №26 TLRR — `wire-in-P0`, baseline 0% зафиксирован живым прогоном |
| Outcome после реализации | `mixed` | №1–23 не аудированы; №24/№25 — analyses provisional; №26 — analysis complete (E2 executed), implementation ⬜ |
...
| 23 | Синтез режимов: «Не меню режимов, а одна учебная нить» | готово (2026-07-18; ревизия плана 2026-07-19; контр-аудит + фикс 2026-07-19) | [`23_one_calm_route.html`](23_one_calm_route.html) | [`../../next/one_calm_learning_route_plan.md`](../../next/one_calm_learning_route_plan.md) | ✅ A1/A2/B1/B2 shipped: A1 «Одна правда маршрута» — SSR как каноническая Route Policy; A2 «Намерение вместо режима» — один reusable route shell, 7 intents; B1 «Checkpoint после каждого результата» — `app/ui/checkpoint.py`, `checkpoint_offered`, tutor/quiz/flashcards integration, stable UUID + `completion_key`; B2 «Учебный компас» — `app/ui/learning_compass.py`, 5 surfaces: home/tutor/plan/quiz/flashcards. Проверка B1+B2: 121 passed. ⬜ Остаток: B3 «Безопасный автопилот» и B4 «Агент из отдельной двери» P1; C1–C2 P2 только после baseline событий. |
...
в связке 9/9 структурно валидных генераций). Следующий пользовательский P0 — №27
«свой материал = полноценный урок». Номера присваиваются окончательно при
выполнении.
...
> **Срез 2026-07-19 после #23 B1+B2.**  Строки #18–#23 выше пересверены с `hometutor` runtime
...
| Разбор | Документ | Статус 2026-07-19 после #23 B1+B2 | Следующее решение |
|---|---|---|---|
...
| #23 One Calm Route | [`one_calm_learning_route_plan.md`](../../next/one_calm_learning_route_plan.md) | ✅ A1/A2/B1/B2 shipped; B1+B2 targeted bundle: 121 passed | owner ack на B1+B2, затем B3 «Безопасный автопилот» → B4 |
...
|---|---|---|
| **#23 B3 «Безопасный автопилот»** | ⬜ ожидает owner ack на B1+B2 | После checkpoint и compass можно дать opt-in budget 5/15/25 мин, но каждый шаг всё равно останавливается на checkpoint и запускается только кликом. |
...
**Рекомендуемый порядок:** owner ack на #23 B1+B2 → #23 B3 → #23 B4 → #19 P1.
...
#19 P0, #22 P0/P1 и #23 A1/A2/B1/B2 тоже закрыты на срезе 2026-07-19 после B1+B2.
Особенно важно: #18 игровая петля, #20 Мнемополис world, #21 мега-бандл и #23 B1/B2 больше не являются текущим P0/P1 backlog.

### Сводка по приоритетам (2026-07-19 после #23 B1+B2)
