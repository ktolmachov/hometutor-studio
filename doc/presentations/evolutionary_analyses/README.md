...
|---|---|---|
| Wiring-status North star | `mixed` | №1–23 не аудированы; №24 — `wire-in-P0`; №25 SGAR — `not measurable` до E2; №26 TLRR — `wire-in-P0`, live-validated baseline 0% (2026-07-23: live-replay + 2 раунда аудит-фиксов P1+P2; `fact_source_binding` fail-closed для None; `grounded_explanation` снят со structural 0 и честно оформлен как alias `evidence_span`; 98 тестов); №27 VLCR — `wire-in-P0`, baseline `N/A` (0 authoritative-прогонов; знаменатель равен 0) |
| Outcome после реализации | `mixed` | №1–23 не аудированы; №24/№25 — analyses provisional; №26 — implementation dry-run-observed + 2 раунда audit-fixes + grounded-alias refresh (2026-07-23: live-replay, 8/10 finding'ов закрыты, `fact_source_binding` None → fail-closed, `grounded_explanation ≡ evidence_span`, 98 тестов; full TLRR остаётся 0% из-за `verified_quiz=0`, а не из-за structural gap); №27 — analysis complete (E2 executed, план v1.7), implementation ⬜ |
...
| 26 | Гейт учебного контента: компилятор доверенного обучения из мега-бандла (связка №24+№25) | **готово, E2 выполнен; implementation dry-run-observed + 2 раунда аудит-фиксов + grounded-alias refresh (2026-07-23)** — P0-A + P0-B закрыты: `course_content_gate_report.json` sidecar, точный evidence-binding квизов, `quiz_results.origin`/`evidence_bound`, TLRR counting. **Раунд 1**: freshness_label по best-source (не any-competitor), `evidence_span_rate` ↔ `evidence_span_presence_rate` имена разделены, micro-quiz без origin → fail-closed, scoped save failure → UI не лжёт, `get_active_generation_view()` вместо ad-hoc parse, `_doc_paths_for_concept` union `documents ∪ related_documents`. **Раунд 2**: `fact_source_binding.apply_quiz_outcome_to_learner_state` fail-closed для `evidence_bound=None`, тест `_none_blocks_mastery` инвертирован. **Раунд 3**: `grounded_explanation` снят со structural 0 через невакуумный `label_mention`; компонент честно зафиксирован как alias `evidence_span`, `tlrr_excluding_grounded` deprecated. 98 тестов (58 gate + 20 TLRR + 20 quiz contract). | [`26_content_gate_compiler.html`](26_content_gate_compiler.html) | [`../../next/course_content_gate_compiler_plan.md`](../../next/course_content_gate_compiler_plan.md) · [`../../next/26_course_content_gate_implementation_report.md`](../../next/26_course_content_gate_implementation_report.md) | ✅ P0-A (Course Content Gate Packet): детерминированные ярлыки роли/свежести/шума/practice, best-source selection с причиной, read-only sidecar рядом с `graph_quality_report.json`, UI-блок «почему этот фрагмент» в reader. ✅ P0-B (Verified Learning Step Contract): `QUIZ_SCOPED_PROMPT` → `source_quote`; exact-match цитат в generation context → `evidence_bound`; аддитивные колонки `quiz_results.origin`/`evidence_bound`; fallback → не пишет mastery; TLRR из DB + gate report. ✅ Grounded-alias refresh: `grounded_explanation ≡ evidence_span` (`label_mention`), invariant-тест закреплён. Post-ship replay боль-якоря «уголовное наказание…»: без evidence не должен пройти в mastery — подтверждён. ⬜ Outcome gap: full TLRR = 0% из-за `verified_quiz=0` на live rows (0/225 evidence-bound), не из-за structural gap. |
...
| #26 Course Content Gate | [`course_content_gate_compiler_plan.md`](../../next/course_content_gate_compiler_plan.md) · [`26_course_content_gate_implementation_report.md`](../../next/26_course_content_gate_implementation_report.md) | ✅ P0-A + P0-B **dry-run-observed + 2 раунда аудит-фиксов + grounded-alias refresh** (2026-07-23): live-replay, 8/10 finding'ов owner-аудита закрыты, `fact_source_binding` fail-closed для None, `grounded_explanation ≡ evidence_span`, 98 тестов; runtime `docs/user_guide.md` и `docs/26_course_content_gate_implementation_report.md` обновлены. ⬜ Outcome gap: full TLRR остаётся 0% из-за `verified_quiz=0` на live rows (0/225 evidence-bound). ⬜ Deferred: P2 scoped concept identity vs gate concept_id; P3 label_mention min-length; P3 UI evidence status до Finish. | Structural residual закрыт; следующий смысловой шаг — добиться хотя бы одного real scoped quiz с exact-match `source_quote` и `evidence_bound=1`; semantic entailment (#25) P1/P2 offline |
...
**#26 P0-A + P0-B полностью закрыты 2026-07-20; 2 раунда аудит-фиксов (P1+P2 + fact_source_binding None→fail-closed) закрыты 2026-07-22.**
Особенно важно: #18 игровая петля, #20 Мнемополис world, #21 мега-бандл и #23 больше не являются текущим P0/P1 backlog.

### Сводка по приоритетам (2026-07-23, после #26 grounded-alias refresh)

| Категория | Разборы |
|---|---|
| **P0 готово / core shipped** | #1–#23, #26. №24/№25 остаются provisional analyses; их runnable safety/evidence часть частично закрыта через №26, но сами P0-кандидаты не объявлены shipped. |
...
| **Опционально / по решению владельца** | #15 B2, #16 P2, #20 metrics/live-polish, #21 manual DoD/UI pin, #26 surface-polish |
...
| Core shipped | 24/27 = **88.9%** |
| В активной работе / outcome gap | 1/27 = **3.7%** |
| Не реализовано как implementation | 3/27 = **11.1%** |
| Средняя готовность по всем разборам | **92.6%** |
| Взвешенная готовность с учётом важности | **~91.6%** |
...
| 26 | Course Content Gate | **85%** | Критическая | Код закрыт; outcome gap: `verified_quiz=0` |
| 27 | Local Model Trust Contour | **35%** | Критическая | Анализ/план готовы; implementation не начата |

Для #26 текущая честная метрика: `grounded_explanation` = 14/18 (77.8%,
alias `evidence_span`), `verified_quiz` = 0/18, live quiz rows = 0/225
`evidence_bound`, full TLRR = 0.0%.

### Открытые хвосты по важности (срез 2026-07-23)

| Приоритет | Хвост | Тип | Важность | Что считается закрытием |
|---|---|---|---|---|
| P0 | **#26 verified_quiz live path** | Product/data outcome | Критическая | Реальный scoped quiz на live bundle сохраняет хотя бы один результат с exact-match `source_quote` и `evidence_bound=1`; TLRR пересчитан, `verified_quiz > 0` |
...
| Optional | **#20 metrics/live-polish** | Observability / UX polish | Средняя | Wired observational metrics `time-to-first-action`, `hall_returns`; live/SR acceptance выполнен |
...
Рекомендуемый порядок: сначала #26 `verified_quiz` как ближайший product/outcome
блокер TLRR, затем #27 как критический engineering trust contour. #25 лучше
не начинать до отдельного решения по semantic eval, чтобы не смешивать его с
уже закрытым deterministic evidence-gate #26.

### Промпт продолжения реализации (срез 2026-07-23)

Скопировать в новую сессию агента перед началом следующей волны:
...
- #1-#23 и #26 core shipped; #24/#25 остаются provisional analyses, их runnable safety/evidence часть частично закрыта через #26.
- #26 structural residual по grounded_explanation закрыт: grounded_explanation ≡ evidence_span (label_mention), tlrr_excluding_grounded deprecated, 98 тестов проходят.
- #26 outcome gap открыт: full TLRR = 0% потому что verified_quiz=0 на live rows (0/225 evidence-bound), не потому что grounded_explanation unwired.
...
1. Закрыть #26 outcome gap: провести real scoped quiz на live bundle так, чтобы вопрос имел exact-match `source_quote`, результат сохранился с `evidence_bound=1`, а пересчёт TLRR показал `verified_quiz > 0`.
2. Не подделывать строки в БД и не засчитывать synthetic/fake progress: это product/data-side проверка живого пути.
3. Если в live path обнаружится баг сохранения `origin`/`evidence_bound` или mismatch concept aliases — чинить минимально в runtime и подтверждать targeted tests.
4. После #26 outcome отдельной волной идти в #27 P0-1…P0-3: Authoritative Trigger Hardening, Runner v0, executable finalize-review.

Ограничения:
- Не начинать #27 без явного owner go после #26 verified_quiz.
...
- Для #26: live `scripts/compute_trusted_route_rate.py` до/после; `.\.venv\Scripts\python.exe -m pytest tests\test_trusted_route_rate.py tests\test_course_content_gate.py tests\test_quiz_content_contract.py`; в отчёте явно указать `verified_quiz`, `quiz_evidence_bound_count`, `quiz_total`.
