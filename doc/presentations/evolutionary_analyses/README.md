...
|---|---|---|
| Wiring-status North star | `mixed` | №1–23 не аудированы; №24 — `wire-in-P0`; №25 SGAR — `not measurable` до E2; №26 TLRR — `wire-in-P0`, live-validated baseline 0% (2026-07-22: live-replay + 2 раунда аудит-фиксов P1+P2, `fact_source_binding` fail-closed для None, 96 тестов; grounded_explanation structural 0); №27 VLCR — `wire-in-P0`, baseline `N/A` (0 authoritative-прогонов; знаменатель равен 0) |
| Outcome после реализации | `mixed` | №1–23 не аудированы; №24/№25 — analyses provisional; №26 — implementation dry-run-observed + 2 раунда audit-fixes (2026-07-22: live-replay, 8/10 finding'ов закрыты, `fact_source_binding` None → fail-closed, 96 тестов; grounded_explanation structural 0 → wave-26-grounded-explanation pending); №27 — analysis complete (E2 executed, план v1.6), implementation ⬜ |
...
| #26 Course Content Gate | [`course_content_gate_compiler_plan.md`](../../next/course_content_gate_compiler_plan.md) · [`26_course_content_gate_implementation_report.md`](../../next/26_course_content_gate_implementation_report.md) | ✅ P0-A + P0-B **dry-run-observed + 2 раунда аудит-фиксов** (2026-07-22): live-replay, 8/10 finding'ов owner-аудита закрыты, `fact_source_binding` fail-closed для None, 96 тестов; раунтайм-срез README + user_guide обновлены. ⬜ Residual: grounded_explanation = structural 0 (очередь wave-26-grounded-explanation). ⬜ Deferred: P2 scoped concept identity vs gate concept_id; P3 label_mention min-length; P3 UI evidence status до Finish. | Live reindex — сделано; post-ship replay — сделано; wire grounded_explanation → full TLRR 6/6; semantic entailment (#25) P1/P2 offline |
...
### Сводка по приоритетам (2026-07-22, после #26: 2 раунда аудит-фиксов)

| Категория | Разборы |
|---|---|
| **P0 готово / core shipped** | #1-#26 |

| **P1 открыт** | #11 пересъёмка скриншотов (content-only, вне runtime scope); **#26 residual** — wire `grounded_explanation` → full TLRR 6/6 |

| **P2 открыт** | нет — все закрыты |
| **Deferred (P2/P3, не блокеры)** | #26: scoped concept identity ≠ gate concept_id; label_mention min-length; UI evidence status до Finish |
| **Контент перед внешним показом** | #11 пересъёмка 06/30 |
| **Опционально / по решению владельца** | #15 B2, #16 P2, #20 metrics/live-polish, #21 manual DoD/UI pin, #26 surface-polish (residual-3) |
