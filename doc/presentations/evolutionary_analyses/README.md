...
|---|---|---|
| Wiring-status North star | `mixed` | №1–23 не аудированы; №24 — `wire-in-P0`; №25 SGAR — `not measured` (E2 feasibility: 0/15 valid, но SGAR baseline не измерен); №26 TLRR — `wire-in-P0`, live-validated baseline 0% (2026-07-23: live-replay + 2 раунда аудит-фиксов P1+P2; `fact_source_binding` fail-closed для None; `grounded_explanation` снят со structural 0 и честно оформлен как alias `evidence_span`; 98 тестов); №27 VLCR — `wire-in-P0`, baseline `N/A` (0 authoritative-прогонов; знаменатель равен 0) |
