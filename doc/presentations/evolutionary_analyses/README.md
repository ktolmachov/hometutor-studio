...
| #26 Course Content Gate | [`course_content_gate_compiler_plan.md`](../../next/course_content_gate_compiler_plan.md) · [`26_course_content_gate_implementation_report.md`](../../next/26_course_content_gate_implementation_report.md) | ✅ P0-A + P0-B **dry-run-observed + 2 раунда аудит-фиксов + grounded-alias refresh** (2026-07-23): live-replay, 8/10 finding'ов owner-аудита закрыты, `fact_source_binding` fail-closed для None, `grounded_explanation ≡ evidence_span`, 98 тестов; runtime `docs/user_guide.md` и `docs/26_course_content_gate_implementation_report.md` обновлены. ⬜ Outcome gap: full TLRR остаётся 0% из-за `verified_quiz=0` на live rows (0/217 evidence-bound). ⬜ Deferred: P2 scoped concept identity vs gate concept_id; P3 label_mention min-length; P3 UI evidence status до Finish. | Structural residual закрыт; следующий смысловой шаг — добиться хотя бы одного real scoped quiz с exact-match `source_quote` и `evidence_bound=1`; semantic entailment (#25) P1/P2 offline |

#### 🔴 P1 — следующий кодовый фокус

| Ход | Статус | Почему следующий |
|---|---|---|
| **#27 P0-1…P0-3 «Execution Packet trust contour»** | ⬜ implementation не начата; файлы runner/policy/finalize отсутствуют | Это следующий настоящий engineering-P0: убрать trust inversion локального coding-executor через task-authored policy, runner, ledger и исполнимый finalize-review. |
...
**Рекомендуемый порядок:** сначала #27 P0-1…P0-3, затем отдельная outcome-волна #26
на real evidence-bound scoped quiz; #11 переснимать перед внешним показом. #22 C1-C3,
...
| **P1 открыт** | #11 пересъёмка скриншотов (content-only, вне runtime scope); **#27 P0-1…P0-3** — Execution Packet trust contour; **#26 outcome gap** — real scoped quiz with exact-match `source_quote` → `evidence_bound=1` |
...
- #26 outcome gap открыт: full TLRR = 0% потому что verified_quiz=0 на live rows (0/217 evidence-bound), не потому что grounded_explanation unwired.
...
4. После #27 core можно отдельной волной закрывать #26 outcome gap: real scoped quiz на live bundle, exact-match source_quote, evidence_bound=1, пересчёт TLRR.
...
- Для #26, если трогаешь: .\.venv\Scripts\python.exe -m pytest tests\test_trusted_route_rate.py tests\test_course_content_gate.py tests\test_quiz_content_contract.py
```
