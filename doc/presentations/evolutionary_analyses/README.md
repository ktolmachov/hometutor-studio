...
baseline честный **1/27 ≈ 3.7%** (только №26 с live TLRR 61.1%), target ≥21%, статус `wire-in-P0`.
...
| Outcome после реализации | `mixed` | №1–23 не аудированы; №24/№25 — analyses provisional; №26 — implementation verified (2026-07-23: TLRR 61.1%, 11/18 full steps, 11/236 evidence-bound quiz rows; remaining 7 steps are content-side residuals: 4 без `label_mention`, 3 без discriminating selection); №27 — analysis complete (E2 executed, план v1.7), implementation ⬜; №28 — analysis complete (E2: живой пересчёт git/реестров 2026-07-24), implementation ⬜ |
...
| 28 | Обзор с высоты пройденного пути (метаразбор итогов серии: «Строили вглубь — фасад остался прежним») | **готово, E2 выполнен (2026-07-24)**: живой пересчёт git-истории runtime с 2026-07-10 (306 коммитов, `app/` +35 524/−3 464 строк, `app/ui` +22 639/−2 739, 29 новых UI-файлов, **0 удалённых**) и реестров (26 `FeatureSpec`, 18 nav-вью) | [`28_summit_retrospective.html`](28_summit_retrospective.html) | [`../../next/series_summit_validation_plan.md`](../../next/series_summit_validation_plan.md) · replay: [`../../next/replay_artifacts_2026-07/replay_2026-07-24.md`](../../next/replay_artifacts_2026-07/replay_2026-07-24.md) | ✅ P0-2 «Диета поверхности» shipped 2026-07-24: `get_ui_level()` (`hometutor@589636dab:app/ui_preferences.py`) больше не автоапгрейдит активный профиль до `diagnostic`; `should_offer_first_choice()`/`is_ui_level_decided()` добавлены; one-time баннер «Простой/Полный вид» на Mission Control (`_render_first_level_choice_banner`); существующий 3-пресетный переключатель (`control_panel.py`) не тронут. ✅ P0-1 infra shipped: checkpoint acceptance-rate — join уже писавшихся `checkpoint_offered`↔`route_selected(accepted=True)` по `decision_id` (`scripts/compute_checkpoint_acceptance_rate.py`); world time-to-first-action потребовал **новую минимальную инструментацию** через существующий session_tape pipeline (`world_entered`/`world_first_action` в новом size-extract модуле `app/ui/dashboards_graph_world_events.py`, вызывается из двух точек `dashboards_graph.py::_render_mnemo_polis_surface`; событий добавлено в `EVENT_REQUIRED_FIELDS` в `session_tape.py`; `scripts/compute_world_time_to_first_action.py`) — честная поправка к самому плану: он ошибочно считал сигнал «уже пишущимся», реально ни одно из событий не существовало до сегодняшнего дня (0 инструментации в `dashboards_graph.py`/`knowledge_graph_d3.py` до этой волны). Извлечение в отдельный модуль потребовалось, чтобы не пробить architecture guard №12 (`peak_file_lines` бюджет 1958 — inline-добавление подняло `dashboards_graph.py` до 1996 строк; после extract — ровно 1958, guard зелёный). Живой прогон обоих скриптов на реальном `DATA_DIR/sessions/`: checkpoint acceptance `N/A` (0 `checkpoint_offered` в истории — реальные сессии используют SSR primary CTA вне чекпоинта), world time-to-first-action `N/A` (0 `world_entered` — сигнал только что подключён). ✅ Replay #26: живой TLRR **61.1%** переподтверждён (бит-в-бит с 2026-07-23 baseline, 112 тестов). ⬜ Replay #2/№19/№22/№23: выполнен только как *mechanical re-verification* (целевые regression-bundles зелёные — 59+54+7+159=279 тестов), явно НЕ поведенческая валидация (нет живого прохода реального пользователя/секундомера/SQL-снимка) — статус `mechanically-reverified`, не засчитан в OVR. **OVR = 1/28 ≈ 3.6%** (было 1/27≈3.7%; знаменатель вырос с добавлением №28, числитель не изменился — mechanical re-verification намеренно не путается с outcome-валидацией). Боль-якорь §4 (E2): причина тройная — `feature_registry.py::FEATURES` «аддитивный слой», 0 удалённых вью, автоапгрейд уровня; P0-2 устраняет именно третью причину (структурно), поведенческое подтверждение («стало ли легче владельцу») ещё не проведено → PAIN-03 инстанс переведён `open → mitigated` (не `closed`). #27 engineering-трек не меняется и идёт параллельно. |
...
| **P0 открыт (два параллельных трека)** | Продуктовый (только остаток #28): полный **поведенческий** replay №2/№19/№22/№23 — сегодняшний прогон дал лишь `mechanically-reverified` (regression-тесты), OVR всё ещё 3.6%, target ≥21% не достигнут. Инженерный: **#27 P0-1…P0-3** — Execution Packet trust contour, не начат. Треки независимы, не смешивать |
...
| Средняя готовность по всем разборам | **~92.9%** |
| Взвешенная готовность с учётом важности | **~92–93%** (оценочно, без опубликованной весовой формулы; критические #25/#27 тянут вниз, #26 поднят после verified_quiz unblock, #28 поднят после P0-1/P0-2) |
...
| 28 | Обзор с высоты пройденного пути | **90%** | Критическая | P0-2 UX-дефолт + P0-1 infra (checkpoint/world сигналы) shipped и протестированы 2026-07-24; replay #26 validated, replay #2/#19/#22/#23 mechanically-reverified only; остаток — полный поведенческий replay |
...
|---|---|---|---|---|
| P0 (продуктовый трек, остаток) | **#28 полный поведенческий replay №2/№19/№22/№23** | Outcome validation | Критическая | P0-2 (study-дефолт) и P0-1 infra (оба сигнала wired, replay-скрипты) shipped 2026-07-24 — закрыто. Остаток: живой прогон на реальном профиле/секундомере/SQL-снимке вместо только regression-тестов; OVR ≥21% в README (сейчас 3.6%, mechanical re-verification не считается) |
...
**Трек A (продуктовый) — #28 P0-1/P0-2, P0-2 и P0-1 infra закрыты 2026-07-24; остаток — поведенческий replay:**
...
Текущий подтверждённый статус (2026-07-24):
- #28 P0-2 «Диета поверхности» ЗАКРЫТ: get_ui_level() больше не автоапгрейдит активный профиль до diagnostic; should_offer_first_choice()/is_ui_level_decided() добавлены; one-time баннер на Mission Control (_render_first_level_choice_banner); тесты зелёные.
- #28 P0-1 infra ЗАКРЫТ: checkpoint acceptance-rate читает уже писавшиеся события (scripts/compute_checkpoint_acceptance_rate.py); world time-to-first-action потребовал новую минимальную инструментацию через существующий session_tape pipeline (world_entered/world_first_action в app/ui/dashboards_graph_world_events.py, scripts/compute_world_time_to_first_action.py) — план ошибочно считал сигнал уже писавшимся; extract в отдельный модуль потребовался, чтобы не пробить architecture guard №12 peak_file_lines. Оба живых прогона честно N/A (0 исторических событий).
- Replay #26 — validated: живой TLRR 61.1% переподтверждён, бит-в-бит с 2026-07-23, 112 тестов.
- Replay №2/№19/№22/№23 — только mechanically-reverified (279 regression-тестов зелёные), НЕ поведенческая валидация. Это остаток P0-1.
- OVR = 1/28 ≈ 3.6% (не 21% — mechanical re-verification намеренно не засчитана).

Цель ближайшей волны — единственная оставшаяся задача P0-1:
...
2. Каждому — честный outcome-статус validated/no-effect/regressed (не mechanically-reverified) с артефактом в doc/next/replay_artifacts_2026-07/.
3. Пересчитать OVR; если поднялся — обновить оба README.
...
- Для каждого из 4 пунктов — конкретное число/наблюдение, не просто "тесты прошли".
