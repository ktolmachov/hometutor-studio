...
№27 — первый инженерный (не продуктовый) разбор с E2-подтверждением после №12; его North
star (VLCR) baseline зафиксирован живым прогоном как честный 0%. Для новых разборов статус
метрики обязателен по правилу `North star wired` из гайда.

| Центральный контроль | Текущий статус | Охват |
|---|---|---|
| Wiring-status North star | `mixed` | №1–23 не аудированы; №24 — `wire-in-P0`; №25 SGAR — `not measurable` до E2; №26 TLRR — `wire-in-P0`, baseline 0% зафиксирован живым прогоном; №27 VLCR — `wire-in-P0`, baseline 0% зафиксирован живым прогоном триггера |
...
| 27 | Протокол суда пишет обвиняемый: контур доверия к локальной coding-модели (инженерный разбор, вне продуктовой UX-карты — аналог №12) | **готово, E2 выполнен; план v1.1 после контр-аудита** (2026-07-19): 4 живых прогона `llamacpp_agent_trigger.ts` против запущенного production-сервера (default alias → `exit 2` за 54мс; corrected alias → полный PASS за 2860мс, evidence-only contract) + 98/98 unit-тестов + 9 стартовых гипотез перепроверены (все CONFIRMED) | [`27_local_model_trust_contour.html`](27_local_model_trust_contour.html) | [`../../next/local_model_execution_packet_plan.md`](../../next/local_model_execution_packet_plan.md) | ⬜ Кандидаты P0-1 (Relocation + Authoritative Trigger Hardening) + P0-2 (Execution Packet Runner v0 + ledger + ADR), не backlog entries. Живой боль-якорь: trust inversion — `validatePatchAgainstWriteSet`/`extractTestCommands` сверяют diff/tests с декларацией самой модели, не с задачей; alias по умолчанию (`qwen/qwen3-coder-next`) не совпадает с production (`qwen3-coder-next-q4ks`). North star `Verified Local Completion Rate` (формула зафиксирована, target ≥80% = KPI guide §8): baseline честный 0% (6 исторических прогонов — все smoke на старом alias), wire-in-P0. Контр-аудит 2026-07-19: 14/15 замечаний принято и внесено в план v1.1 (перенос контура в `hometutor/scripts/agent_exec/`, per-run receipts + `run_id` вместо last-row, `packet_policy.ts` с repo-allowlist/realpath containment/встроенным денилистом, cap 20K по `token_safety_registry.json:6`, read-set c sha256, ADR о сосуществовании со studio-SSoT, quality-fallback запрещён, исход `blocked_no_changes`, baseline с untracked, снято противоречие о доступе локальной модели к Runner); замечание про `.kilo/kilo.jsonc` отклонено частично — файл существует и именно он задаёт модель, корневой `kilo.jsonc` содержит только instructions. PAIN-02 инстанс добавлен в реестр. |
...
> текущий кодовый фокус — #23 B3/B4 и #19 P1, затем P2-хвосты #22/#23/#19.
...
| #22 Progress Mirror | [`progress_mirror_plan.md`](../../next/progress_mirror_plan.md) | ✅ P0/P1 shipped: A1/A2 + B1–B3 закрыты | C1–C3 P2: аналитика / emotion layer KG / real `user_id` |
| #23 One Calm Route | [`one_calm_learning_route_plan.md`](../../next/one_calm_learning_route_plan.md) | ✅ A1/A2/B1/B2 shipped и owner-accepted; финальная проверка: targeted bundle 138 passed, architecture/checkpoint/compass 63 passed | B3 «Безопасный автопилот» → B4 |
| #19 Lecture Presence | [`lecture_presence_checkpoints_plan.md`](../../next/lecture_presence_checkpoints_plan.md) | ✅ P0-1/P0-2 shipped | P1 ставка до отрезка + глубина; P2 скука без признания |
...
**Рекомендуемый порядок:** #23 B3 → #23 B4 → #19 P1.
#22 C1–C3, #19 P2 и #23 C1–C2 — P2 после P1-кандидатов или по явному owner ack.
...
| **P2** | **#23 C1–C2** | ⬜ калибровка Route Policy и адаптивная глубина только после baseline событий |
...
#19 P0, #22 P0/P1 и #23 A1/A2/B1/B2 тоже закрыты на срезе 2026-07-19 после owner ack B1+B2.
Особенно важно: #18 игровая петля, #20 Мнемополис world, #21 мега-бандл и #23 B1/B2 больше не являются текущим P0/P1 backlog.

### Сводка по приоритетам (2026-07-19 после owner ack #23 B1+B2)

| Категория | Разборы |
|---|---|
| **P0 готово / core shipped** | #1–#10, #12–#23 |
| **P1 открыт** | #23 B3–B4, #19 P1 |
| **P2 открыт** | #22 C1–C3, #19 P2, #23 C1–C2 |
