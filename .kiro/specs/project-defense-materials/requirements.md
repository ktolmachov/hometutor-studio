...
Фича создаёт два документа для академической защиты проектной работы по проекту **home-rag_v2**:
...
3. WHEN слайд обзора создаётся, THE Defense_Presentation SHALL указывать конкретные инструменты, которые заменяет home-rag (ChatGPT для вопросов, Anki для карточек, Obsidian для заметок).
