...
- **≤ 12k токенов** — целевой input на один LLM-ход.
- **12k–20k** — сжать read-set и history; не добавлять новые full-read.
- **> 20k** — стоп, отчёт о блокере; **новый чат**, если `msgs`≫15 или длинная tool-heavy сессия (relay guard hard ≈110k chars body).
- **Read-set: max 3–5 файлов** (рутина — 2–3). Только из явного read-set задачи.
- **Never full-read через Read tool** (уже в `<rules>` / always-on): `AGENTS.md`, `CLAUDE.md`, `doc/backlog_registry.yaml`, `doc/closed_iterations.md`, `doc/conventions_architecture.md`, `doc/conventions_reference.md`, `doc/epochs/*`, крупные `tests/test_*.py` — только `rg`/секция по `safe_hint` в registry.
- **Не дублировать** always-on policy: не читать `AGENTS.md`/`CLAUDE.md` целиком «для контекста» — только нужный заголовок через `rg`.
